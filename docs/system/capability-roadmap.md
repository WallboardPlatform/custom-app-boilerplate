...
| Weather | Supported SDK `useWeather`/`WeatherService`; no datasource binding | Platform `getWeatherData`, `cacheFile`, production condition vocabulary, location-keyed preview mock | Prove deterministic no-data/error/icon fallback in a focused generated app; never call vendor APIs or own credentials |
