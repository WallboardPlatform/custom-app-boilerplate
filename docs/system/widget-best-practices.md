...
The standard editor property schema does not rewrite sibling color values when a preset select changes. Keep manual color controls beside `themePreset` in the same first-level group and put the `themePreset === "custom"` `visibilityConditions` rule on every swatch. Do not nest them in a `Custom colors` group: the legacy editor may render only the nested group label. Curated presets must also replace every semantic surface, divider, track, and status color. Avoid dark-specific translucent literals outside the resolved palette because they frequently destroy contrast in the light preset.
