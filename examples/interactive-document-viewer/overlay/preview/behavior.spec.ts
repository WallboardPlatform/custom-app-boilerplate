...
test('tapping the directory search opens the compact keyboard in its responsive placement', async ({
	page
}): Promise<void> => {
	for (const testCase of [
		{ scenario: 'landscape', placement: 'directory' },
		{ scenario: 'portrait', placement: 'page' }
	] as const) {
		await openScenario(page, testCase.scenario, false);
		const root = page.locator('[data-preview-id="interactive-document-viewer-root"]');
		const search = page.getByLabel('Search the document directory');
		...
		await expect(searchFrame).toHaveCSS('border-top-width', '2px');
		await expect(searchFrame).toHaveCSS('border-top-style', 'solid');
		await expect(searchFrame).toHaveCSS('border-top-color', 'rgb(201, 213, 226)');
		const keyboard = page.getByRole('dialog', { name: 'On-screen keyboard' });
		await expect(keyboard).toBeVisible();
		await expect(keyboard).toHaveAttribute('data-placement', testCase.placement);
		await expect(root).toHaveAttribute('data-keyboard-open', 'true');
		await expect(root).toHaveAttribute('data-keyboard-placement', testCase.placement);
		await expect(keyboard.locator('[data-preview-id="keyboard-header"]')).toHaveCount(0);

		const close = keyboard.getByRole('button', { name: 'Close', exact: true });
		await expect(close).toBeVisible();
		await expect(close.locator('..')).toHaveJSProperty('tagName', 'FOOTER');
		const compactGeometry = await keyboard.evaluate(
			(
				element: HTMLElement
			): {
				footerBelowKeys: boolean;
				...
				if (!panel || !keys || !footer) throw new Error('Keyboard compact-layout target is missing.');
				const panelBounds = panel.getBoundingClientRect();
				const keysBounds = keys.getBoundingClientRect();
				const footerBounds = footer.getBoundingClientRect();

				return {
					footerBelowKeys: footerBounds.top >= keysBounds.bottom,
					footerContained: footerBounds.bottom <= panelBounds.bottom + 1,
					topInset: keysBounds.top - panelBounds.top
				};
			}
		);

		expect(compactGeometry.topInset).toBeLessThanOrEqual(20);
		expect(compactGeometry.footerBelowKeys).toBe(true);
		expect(compactGeometry.footerContained).toBe(true);

		await close.click();
		await expect(keyboard).toHaveCount(0);
		await expect(root).toHaveAttribute('data-keyboard-open', 'false');

		await pushConfiguration(page, { keyboardEnabled: false });
		await search.click();
		await expect(page.getByRole('dialog', { name: 'On-screen keyboard' })).toHaveCount(0);
	}
});

test('directory and keyboard separators sit at their section boundaries', async ({ page }): Promise<void> => {
	for (const scenario of ['landscape', 'portrait']) {
		await openScenario(page, scenario, false);
		const directory = page.getByRole('complementary', { name: 'Document directory' });
		const directoryHeader = directory.locator(':scope > div').first();
		const search = page.getByLabel('Search the document directory').locator('..');
		const directorySeparator = page.locator('[data-preview-id="directory-content-separator"]');
		const documentList = page.locator('[data-preview-id="document-list"]');

		await expect(directoryHeader).toHaveCSS('border-bottom-width', '0px');
		await expect(directorySeparator).toBeVisible();
		await expect(directorySeparator).toHaveCSS('background-color', 'rgb(224, 231, 239)');
		await expect(directorySeparator).toHaveCSS('height', '1px');
		const directoryBounds = await directory.boundingBox();
		const searchBounds = await search.boundingBox();
		const separatorBounds = await directorySeparator.boundingBox();
		const listBounds = await documentList.boundingBox();

		expect(directoryBounds).not.toBeNull();
		expect(searchBounds).not.toBeNull();
		expect(separatorBounds).not.toBeNull();
		expect(listBounds).not.toBeNull();
		if (directoryBounds && searchBounds && separatorBounds && listBounds) {
			expect(separatorBounds.y).toBeGreaterThanOrEqual(searchBounds.y + searchBounds.height - 1);
			expect(separatorBounds.y + separatorBounds.height).toBeLessThanOrEqual(listBounds.y + 1);
			expect(separatorBounds.x).toBeGreaterThanOrEqual(directoryBounds.x);
			expect(separatorBounds.x + separatorBounds.width).toBeLessThanOrEqual(
				directoryBounds.x + directoryBounds.width + 1
			);
			expect(separatorBounds.width).toBeGreaterThanOrEqual(directoryBounds.width - 3);
		}

		await page.getByRole('button', { name: 'Open on-screen keyboard' }).click();
		const keyboard = page.getByRole('dialog', { name: 'On-screen keyboard' });
		const keyboardSeparator = page.locator('[data-preview-id="keyboard-separator"]');
		const keyboardShell = keyboardSeparator.locator('..');
		await expect(keyboardSeparator).toBeVisible();
		await expect(keyboardSeparator).toHaveCSS('background-color', 'rgb(185, 201, 218)');
		await expect(keyboardSeparator).toHaveCSS('height', '2px');
		expect(
			await keyboardSeparator.evaluate((element: HTMLElement): string => getComputedStyle(element).boxShadow)
		).not.toBe('none');

		const listAfter = await documentList.boundingBox();
		const keyboardBounds = await keyboard.boundingBox();
		const keyboardSeparatorBounds = await keyboardSeparator.boundingBox();
		const keyboardShellBounds = await keyboardShell.boundingBox();
		expect(listAfter).not.toBeNull();
		expect(keyboardBounds).not.toBeNull();
		expect(keyboardSeparatorBounds).not.toBeNull();
		expect(keyboardShellBounds).not.toBeNull();
		if (listAfter && keyboardBounds && keyboardSeparatorBounds && keyboardShellBounds) {
			expect(keyboardSeparatorBounds.y).toBeGreaterThanOrEqual(listAfter.y + listAfter.height - 1);
			expect(keyboardSeparatorBounds.y + keyboardSeparatorBounds.height).toBeLessThanOrEqual(keyboardBounds.y + 1);
			expect(Math.abs(keyboardSeparatorBounds.x - keyboardShellBounds.x)).toBeLessThanOrEqual(1);
			expect(Math.abs(keyboardSeparatorBounds.width - keyboardShellBounds.width)).toBeLessThanOrEqual(1);
		}
	}
});

test('landscape keyboard stays inside the directory without resizing the PDF reader', async ({
	page
}): Promise<void> => {
	await openScenario(page);
	const root = page.locator('[data-preview-id="interactive-document-viewer-root"]');
	const directory = page.getByRole('complementary', { name: 'Document directory' });
	const reader = page.getByRole('region', { name: 'PDF document viewer' });
	const readerBefore = await reader.boundingBox();
	const directoryBefore = await directory.boundingBox();

	expect(readerBefore).not.toBeNull();
	expect(directoryBefore).not.toBeNull();
	await page.getByRole('button', { name: 'Open on-screen keyboard' }).click();

	const keyboard = page.getByRole('dialog', { name: 'On-screen keyboard' });
	const searchButton = keyboard.getByRole('button', { name: 'Search', exact: true });
	await expect(keyboard).toBeVisible();
	await expect(keyboard).toHaveAttribute('data-embedded', 'true');
	await expect(keyboard).toHaveAttribute('data-placement', 'directory');
	await expect(root).toHaveAttribute('data-keyboard-placement', 'directory');
	...
	const keyboardLayout = await keyboard.evaluate((element: HTMLElement): { background: string; position: string } => ({
		background: getComputedStyle(element).backgroundColor,
		position: getComputedStyle(element).position
	}));
	expect(keyboardLayout.position).toBe('relative');
	expect(keyboardLayout.background).not.toBe('rgba(0, 0, 0, 0.48)');

	const keyboardBounds = await keyboard.boundingBox();
	const directoryAfter = await directory.boundingBox();
	const readerAfter = await reader.boundingBox();
	const listBounds = await page.locator('[data-preview-id="document-list"]').boundingBox();
	expect(keyboardBounds).not.toBeNull();
	expect(directoryAfter).not.toBeNull();
	expect(readerAfter).not.toBeNull();
	expect(listBounds?.height ?? 0).toBeGreaterThan(60);
	if (keyboardBounds && directoryAfter) {
		expect(keyboardBounds.x).toBeGreaterThanOrEqual(directoryAfter.x - 1);
		expect(keyboardBounds.y).toBeGreaterThanOrEqual(directoryAfter.y - 1);
		expect(keyboardBounds.x + keyboardBounds.width).toBeLessThanOrEqual(directoryAfter.x + directoryAfter.width + 1);
		expect(keyboardBounds.y + keyboardBounds.height).toBeLessThanOrEqual(directoryAfter.y + directoryAfter.height + 1);
	}
	if (readerBefore && readerAfter) {
		expect(Math.abs(readerAfter.x - readerBefore.x)).toBeLessThanOrEqual(1);
		expect(Math.abs(readerAfter.y - readerBefore.y)).toBeLessThanOrEqual(1);
		expect(Math.abs(readerAfter.width - readerBefore.width)).toBeLessThanOrEqual(1);
		expect(Math.abs(readerAfter.height - readerBefore.height)).toBeLessThanOrEqual(1);
	}
	if (directoryBefore && directoryAfter) {
		expect(Math.abs(directoryAfter.width - directoryBefore.width)).toBeLessThanOrEqual(1);
		expect(Math.abs(directoryAfter.height - directoryBefore.height)).toBeLessThanOrEqual(1);
	}
...
test('landscape and portrait keyboards share one clean, aligned visual treatment', async ({ page }): Promise<void> => {
	const measurements: KeyboardSurfaceMeasurement[] = [];

	for (const scenario of ['landscape', 'portrait']) {
		await openScenario(page, scenario, false);
		await page.getByRole('button', { name: 'Open on-screen keyboard' }).click();
		await expect(page.getByRole('dialog', { name: 'On-screen keyboard' })).toBeVisible();
		const measurement = await measureKeyboardSurface(page);
		measurements.push(measurement);

		expect(measurement.wrapperBackground).toBe('rgba(0, 0, 0, 0)');
		expect(measurement.wrapperBorderWidth).toBe('0px');
		expect(measurement.wrapperBoxShadow).toBe('none');
		expect(measurement.panelBackground).toBe('rgb(255, 255, 255)');
		expect(measurement.panelBorderWidth).toBe('0px');
		expect(measurement.panelRadius).toBe(measurement.directoryRadius);
		expect(Number.parseFloat(measurement.panelRadius)).toBeGreaterThanOrEqual(8);
		expect(measurement.controlRadius).toBe(measurement.searchFieldRadius);
		expect(measurement.keyRadius).toBe(measurement.searchFieldRadius);
		expect(measurement.closeBackground).toBe(measurement.keyBackground);
		expect(measurement.closeBorderColor).toBe(measurement.keyBorderColor);
		expect(measurement.closeBoxShadow).toBe('none');
		expect(measurement.closeOutlineStyle).toBe('none');
		expect(measurement.searchBackground).toBe('rgb(10, 45, 111)');
		expect(measurement.searchColor).toBe('rgb(255, 255, 255)');
	}

	expect(measurements[0].panelBackground).toBe(measurements[1].panelBackground);
	expect(measurements[0].panelBoxShadow).toBe(measurements[1].panelBoxShadow);
	expect(measurements[0].panelRadius).toBe(measurements[1].panelRadius);
});

test('keyboard uses uppercase labels, omits redundant controls, and title-cases entered words', async ({
	page
}): Promise<void> => {
	await openScenario(page, 'landscape', false);
	const searchInput = page.getByLabel('Search the document directory');
	await page.getByRole('button', { name: 'Open on-screen keyboard' }).click();
	const keyboard = page.getByRole('dialog', { name: 'On-screen keyboard' });
	const keys = keyboard.locator('[data-preview-id="keyboard-key"]');

	await expect(keyboard.getByRole('button', { name: 'EN', exact: true })).toHaveCount(0);
	await expect(keyboard.getByRole('button', { name: 'Shift', exact: true })).toHaveCount(0);
	await expect(keyboard.getByText('Search community documents', { exact: true })).toHaveCount(0);
	await expect(keyboard.locator('[data-preview-id="keyboard-value"]')).toHaveCount(0);

	const keyLabels = (await keys.allTextContents()).map((label: string): string => label.trim());
	const letterLabels = keyLabels.filter((label: string): boolean => /^[A-Z]$/u.test(label));
	const accessibleKeyLabels = await keys.evaluateAll((elements: HTMLElement[]): string[] =>
		elements.map((element: HTMLElement): string => element.getAttribute('aria-label') ?? '')
	);
	expect(letterLabels).toHaveLength(26);
	expect(accessibleKeyLabels.filter((label: string): boolean => /^Key [A-Z]$/u.test(label))).toHaveLength(26);
	expect(keyLabels.some((label: string): boolean => /^[a-z]$/u.test(label))).toBe(false);

	for (const key of ['B', 'U', 'D', 'G', 'E', 'T']) {
		await keyboard.getByRole('button', { name: `Key ${key}`, exact: true }).click();
	}
	await keyboard.getByRole('button', { name: 'Space', exact: true }).click();
	for (const key of ['R', 'E', 'P', 'O', 'R', 'T']) {
		await keyboard.getByRole('button', { name: `Key ${key}`, exact: true }).click();
	}

	await expect(searchInput).toHaveValue('Budget Report');
	await expect(page.getByText('1 result', { exact: true })).toBeVisible();
	await expect(page.getByRole('button', { name: /Budget Report:/ })).toBeVisible();
});

test('keyboard rows remain evenly centered and contained across responsive surfaces', async ({
	page
}): Promise<void> => {
	const cases: Array<{
		height: number;
		scenario: 'landscape' | 'portrait';
		surface: 'landscape' | 'portrait';
		width: number;
	}> = [
		{ height: 1080, scenario: 'landscape', surface: 'landscape', width: 1920 },
		{ height: 1920, scenario: 'portrait', surface: 'portrait', width: 1080 },
		{ height: 1280, scenario: 'portrait', surface: 'portrait', width: 720 }
	];
	...
	for (const testCase of cases) {
		...
		const root = page.locator('[data-preview-id="interactive-document-viewer-root"]');
		await expect(root).toHaveAttribute('data-surface', testCase.surface);
		await page.getByRole('button', { name: 'Open on-screen keyboard' }).click();
		await expect(page.getByRole('dialog', { name: 'On-screen keyboard' })).toBeVisible();
		const measurement = await measureKeyboardGrid(page);

		expect(measurement.rowCount).toBe(4);
		expect(measurement.contained).toBe(true);
		expect(measurement.horizontalOverflow).toBeLessThanOrEqual(1);
		expect(measurement.centerDelta).toBeLessThanOrEqual(2);
		expect(measurement.keyWidthVariation).toBeLessThanOrEqual(1);
		expect(measurement.keyGapVariation).toBeLessThanOrEqual(1);
	}
});

test('portrait keyboard footer actions evenly fill and align with the key rows', async ({ page }): Promise<void> => {
	...
	for (const size of sizes) {
		...
		await page.getByRole('button', { name: 'Open on-screen keyboard' }).click();
		await expect(page.getByRole('dialog', { name: 'On-screen keyboard' })).toBeVisible();
		const measurement = await measureKeyboardFooter(page);

		expect(measurement.contained).toBe(true);
		expect(measurement.edgeDelta).toBeLessThanOrEqual(1);
		expect(measurement.fillRatio).toBeGreaterThan(0.98);
		expect(measurement.footerKeysLeftDelta).toBeLessThanOrEqual(1);
		expect(measurement.footerKeysWidthDelta).toBeLessThanOrEqual(1);
		expect(measurement.gapVariation).toBeLessThanOrEqual(1);
	}
});

test('keyboard text scale enlarges labels without materially changing key targets', async ({ page }): Promise<void> => {
	await openScenario(page, 'portrait', false);
	const root = page.locator('[data-preview-id="interactive-document-viewer-root"]');
	await page.getByRole('button', { name: 'Open on-screen keyboard' }).click();

	const keyboard = page.getByRole('dialog', { name: 'On-screen keyboard' });
	const letterKey = keyboard.getByRole('button', { name: 'Key A' });
	const measure = async (): Promise<{ fontSize: number; width: number; height: number }> => {
		const box = await letterKey.boundingBox();
		const fontSize = await letterKey.evaluate((element: HTMLElement): number =>
			Number.parseFloat(getComputedStyle(element).fontSize)
		);

		if (!box) throw new Error('Keyboard key has no measurable bounds.');
		return { fontSize, width: box.width, height: box.height };
	};
	const before = await measure();

	await pushConfiguration(page, { keyboardTextScale: 120 });
	await expect(root).toHaveAttribute('data-keyboard-text-scale', '120');
	await expect(keyboard).toHaveAttribute('data-text-scale', '120');
	await expect.poll(async (): Promise<number> => (await measure()).fontSize).toBeGreaterThan(before.fontSize + 2);
