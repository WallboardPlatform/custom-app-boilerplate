...
| `veszprem-wayfinding` | Supplied SVG map, legacy proximity compatibility through the shared route engine, touch directory/detail panel, editable `TABLE` destinations | Full HD, compact, selected route, map click, off-map destination, light/dark/custom, empty, long labels/list, row array, live update, reset |
