...
| `civic-building-directory` | Redrawn-equivalent three-level wayfinding, independent walkable mask, reviewed centerline graph, leaf entrances, aligned elevator transitions, step-free routing | Full HD, compact, all levels, same-floor route, both cross-floor route segments, manual zoom, highlight fallback, keyboard, light/dark/custom, empty, long copy, row array, live update, reset |
...
| `veszprem-wayfinding` | Source-first supplied map, quiet SVG hit targets, evidence-bounded highlight/directional guidance, touch keyboard, metadata details, editable `TABLE` destinations | Full HD, compact, highlight, directional cue, map click, keyboard, off-map destination, light/dark/custom, empty, long labels/list, row array, live update, reset |
