...
| `curated-storyline` | App-owned nested editorial scenes, structured custom settings editor, guarded ordering and publication, passive rotation without a datasource | Full HD, wide/low, portrait, square, quote, schedule, long copy, empty, custom theme, live editor round-trip |
...
| `lumen-media-program` | Opt-in browser video surface, direct file/folder/JSON playlist, native MP4/WebM plus HLS fallback, captions, controls, commands, telemetry, and recovery | Full HD, wide/low, portrait, square, compact, long labels, custom theme, source failure, real playback, cache, commands, teardown |
...
| `pdf-document-workspace` | Opt-in PDF.js document surface, file/folder sources, multiple documents, outline/search/link/annotation/AcroForm layers, navigation, zoom, auto-scroll, download, and external commands | Full HD, compact, portrait, square, wide/low, light/dark/custom, continuous/single page, lazy rendering, forms, remote navigation, search, auto-scroll |
...
| `spatial-wayfinding` | Northline field-tested `.wbmap` runtime, 2D/3D visitor views, multilingual directory, destination details, layer controls, selection, and routed guidance | Full HD, compact, 2D/3D, dark, selected details, search, language, layers, active route, packaged runtime |
| `veszprem-wayfinding` | Manual-first Studio project over supplied artwork, reviewed SVG hit targets, touch keyboard, metadata details, editable `TABLE` destinations, evidence-bounded highlight/directional guidance | Full HD, compact, active highlight, map click, keyboard, off-map destination, light/dark/custom, empty, long labels/list, row array, live update, reset |
