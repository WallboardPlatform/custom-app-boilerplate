...
| `veszprem-wayfinding` | Source-first supplied map, explicit reviewed route graph, SVG hit targets, touch keyboard, metadata details, editable `TABLE` destinations | Full HD, compact, selected route, map click, keyboard, off-map destination, light/dark/custom, empty, long labels/list, row array, live update, reset |
