...
| Build a wayfinding custom app | [`docs/system/wayfinding.md`](docs/system/wayfinding.md) and the canonical vendored viewer contract |
...
Wayfinding uses three distinct artifacts: the editable `.wbwayfinding` source, its generated `.wbmap` runtime package, and the accepted custom-app upload ZIP. Author and publish with [Wayfinding Studio](https://wayfinding.wallboard.info); custom apps compose their visitor shell around the canonical viewer generated with the Editor renderer. See [`docs/system/wayfinding.md`](docs/system/wayfinding.md). Existing wayfinding examples are legacy material, not the implementation reference for new work.
