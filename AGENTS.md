...
For a wayfinding request based on a PDF, image, SVG, or existing map, read `docs/system/wayfinding.md` and author it in [Wayfinding Studio](https://wayfinding.wallboard.info). Studio owns `.wbwayfinding` authoring, `.wbmap` publishing, and the canonical mini-displayer. Add the opt-in `wayfinding` capability and compose the kiosk around its harness; never recreate Preview rendering, hand-edit `.wbmap`, or copy viewer internals into an app. Use `examples/wayfinding-kiosk` as the consumer reference, not as a visual template. Its synthetic map is test evidence only; real apps package their venue-specific Studio export.
