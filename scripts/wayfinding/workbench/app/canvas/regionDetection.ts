...
export const detectFlatRegionBoundary = (
	...
	if (regionSize < 12 || regionSize > columns * rows * 0.92) return undefined;
	...
	const loops = traceBoundaryLoops(region, columns, rows, gridSize);
	const outer = loops[0];

	if (!outer) return undefined;
	const simplifyLoop = (loop: WayfindingPoint[]): WayfindingPoint[] => {
		const simplified = simplifyGeometry(
			loop.slice(0, -1),
			Math.max(gridSize * 1.5, options.detail * 0.55, 3)
		);

		return removeRedundantPolygonPoints(
			removeShallowBoundaryDetails(simplified, gridSize, options.detail),
			Math.max(1, gridSize * 1.25, options.detail * 0.35)
		);
	};
	const geometry = simplifyLoop(outer);
	const minimumHoleArea = Math.max(
		gridSize * gridSize * 12,
		Math.abs(polygonArea(outer)) * 0.0015
	);
	const holes = loops.slice(1)
		.filter((loop) => Math.abs(polygonArea(loop)) >= minimumHoleArea)
		.map(simplifyLoop)
		.filter((loop) => loop.length >= 3);

	return geometry.length >= 3
		? { color: colorToHex(averageColor), geometry, holes }
		: undefined;
};
