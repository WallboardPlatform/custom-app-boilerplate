...
void test('visitor scene honors label and media visibility without hiding locations', () => {
	...
	assert.deepEqual(
		...
		[
			...
			'presentation-destination-marker:destination-information'
		]
	);
	...
	const destinationMarker = projected.floors[0].elements.find(
		(element) => element.id === 'presentation-destination-marker:destination-information'
	);
	assert.equal(destinationLabel?.type, 'label');
	assert.equal(destinationMarker?.type, 'poi');
	assert.equal(
		destinationMarker?.type === 'poi' ? destinationMarker.destinationId : undefined,
		'destination-information'
	);
	assert.deepEqual(
		destinationMarker?.type === 'poi' ? destinationMarker.point : undefined,
		{ x: 200 / 3, y: 100 / 3 }
	);
	...
	assert.deepEqual(
		...
		['location-1', 'poi-1', 'transition-1', 'presentation-destination-marker:destination-information']
	);
});
