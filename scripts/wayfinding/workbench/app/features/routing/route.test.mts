...
void test('route cleanup removes a proportionally tiny real-map origin connector', (): void => {
	const cleaned = cleanRoutePath([
		{ levelId: 'level-0', x: 943.455, y: 401.176 },
		{ levelId: 'level-0', x: 941.5, y: 388.5 },
		{ levelId: 'level-0', x: 850.5, y: 311.5 },
		{ levelId: 'level-0', x: 787.5, y: 297.5 }
	]);
...
void test('route journey preserves ordered floors and explicit transitions', (): void => {
	...
	project.graph.edges.find((edge) => edge.id === 'ground-walk')!.distanceMeters = 999;
	project.graph.edges.find((edge) => edge.id === 'first-walk')!.distanceMeters = 999;
