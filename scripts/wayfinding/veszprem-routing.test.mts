...
import type { WayfindingGraphDocument, WayfindingNode, WayfindingWalkableMaskDocument } from '../../src/utils/wayfinding.js';
...
void describe('Veszprem reviewed route topology', (): void => {
	...
	void it('requires the named crossings and stairs for representative routes', (): void => {
		assert.ok(routeEdges('code-digitalis-elmenykozpont').includes('crossing-ovari-west'));
		assert.ok(routeEdges('petofi-szinhaz').includes('crossing-ovari-east'));
		assert.ok(routeEdges('ruttner-haz-varborton').includes('stairs-jokai-ovaros'));
